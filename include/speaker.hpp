...
public:
    ...
    // Like play_wav(), but writes audio in small chunks and checks should_stop()
    // between chunks.  If should_stop() becomes true, queued audio is flushed.
    bool play_wav_interruptible(
    ...
    // Drop audio already queued in the Pulse/PipeWire stream. Useful when the
    // physical button starts a new query and any current speech should stop.
    bool flush();
