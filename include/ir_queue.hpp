...
class IRQueue {
public:
    IRQueue(std::size_t max_capacity)
        : max_capacity_(max_capacity), stopped_(false)
    {
    }

    bool push(IRPacket frame) { //push a frame into queue
    ...
    std::optional<IRPacket> pop(){ //pop a frame from queue
        ...
        */
        ...
        IRPacket frame = std::move(queue_.front());
