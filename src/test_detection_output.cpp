...
static void print_detections(const std::vector<Detection>& dets, int img_w, int img_h)
{
    ...
    for (const auto& d : sorted) {
        const int x1 = static_cast<int>(d.box.x_min * img_w);
        const int y1 = static_cast<int>(d.box.y_min * img_h);
        const int x2 = static_cast<int>(d.box.x_max * img_w);
        const int y2 = static_cast<int>(d.box.y_max * img_h);
...
int main(int argc, char** argv)
{
    ...
    // Send BGR directly — model expects BGR.
    if (!bgr.isContinuous()) bgr = bgr.clone();
    hailo_prepare(bgr.data, bgr.total() * 3);
    ...
    if (!hailo.write_frame(bgr.data, /*camera_id=*/0, /*timestamp_ns=*/0)) {
        std::cout << "FAILED\n";
    ...
    const auto raw_outputs = result_future.get();
    const auto detections  = parse_detections(raw_outputs);
