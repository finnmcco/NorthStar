                                  ...
                                  const StereoDepthEstimator& est)
{
    ...
    // 2. Rectified side-by-side
    {
        ...
        if (!lr.empty() && !rr.empty()) {
            ...
            std::printf("[save] WARNING: rectified frames not available for pair %d\n", idx);
        }
    }
    ...
    // 3. Disparity map
    {
        ...
        if (!disp.empty()) {
            ...
            std::printf("[save] WARNING: disparity not available for pair %d\n", idx);
        }
    }
}
    ...
    */
    ...
    // ── Diagnostic image output directory (timestamped) ──────────────────────
    {
        ...
        if (ec)
            ...
            std::printf("[main] diagnostic images -> %s/\n", g_save_dir.c_str());
    }
