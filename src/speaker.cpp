...
bool Speaker::flush() {
    if (!pa_) return true;
    int err = 0;
    if (pa_simple_flush(pa_, &err) < 0) {
        err_ = std::string("pa_simple_flush: ") + pa_strerror(err);
        return false;
    }
    return true;
}
...
bool Speaker::play_wav_interruptible(
    ...
    const size_t avail  = wav_data.size() - info.data_offset;
    const size_t nbytes = std::min<size_t>(info.data_size, avail);
    const uint8_t* pcm  = wav_data.data() + info.data_offset;

    // Small chunks keep button interruption responsive even if the audio server
    // write call blocks briefly. 4096 bytes is about 93 ms at Piper's default
    // 22.05 kHz mono/16-bit output.
    ...
    while (off < nbytes) {
        ...
        const size_t todo = std::min(kChunkBytes, nbytes - off);
        int err = 0;
        if (pa_simple_write(pa_, pcm + off, todo, &err) < 0) {
            err_ = std::string("pa_simple_write: ") + pa_strerror(err);
            return false;
        }
        off += todo;
    ...
    // Do not drain here.  Leaving audio queued lets this call return quickly,
    // and a later button press can immediately flush the queued speech.
    return true;
}
